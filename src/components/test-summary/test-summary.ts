...
export class TestSummaryComponent {
  ...
  ngOnInit() {
    ...
    this.etas = Object.keys(this.summaryMetadata.eta)
      .filter((action: string) => this.summaryMetadata.eta[action] ? action : '');
    this.etas.forEach((action: string, index: number) => this.etas[index] = action.charAt(0).toUpperCase());
    // Format ECO selections
    this.summaryMetadata.eco.selections.forEach((selection: string, index: number) => this.summaryMetadata.eco.selections[index] = upperFirst(selection));
  }
}
